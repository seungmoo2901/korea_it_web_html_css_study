...
//이름(키)와 내용(값)을 한 쌍 => JSON문자열 형태

//현제 todo데이터를 로컬스토리지에 저장하는 함수
function saveTodoLocalStorage() {
  localStorage.setItem("todos", JSON.stringify(todos));
  //JSON.stringify => 자바스크립트 객체나 배열을 JSON문자열 형태로 변환
}

//로컬스토리지에서 todo데이터를 불러오는함수
function loadTodoLocalStorage() {
  ...
  if (localStorageTodo) {
    ...
    if (todos.length > 0) {
      ...
      //저장되어 있는 todo를 불러와서 다음 할 일을 추가할때 사용할 Id 설정
      //todo들 중 가장 큰 Id를 가져와서 +1 해줌
    } else {
      nextTodoId = 1;
    }
  } else {
...
function deleteTodo(id) {
  ...
  saveTodoLocalStorage(); // 해당 ID를 제외한 목록으로 갱신
  renderTodo(); // 화면 다시 그림
}
